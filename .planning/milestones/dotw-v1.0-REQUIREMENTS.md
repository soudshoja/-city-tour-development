...
- [x] **BLOCK-01**: GraphQL `blockRates` mutation accepts: hotel_code, dates, room_config, selected_room_type, selected_rate_basis, allocationDetails token, resayil_message_id header
- [x] **BLOCK-02**: Validates allocationDetails token matches selected hotel (prevents token mixing)
- [x] **BLOCK-03**: Calls DOTW getRooms with blocking=true (locks rate for 3 minutes)
- [x] **BLOCK-04**: Creates `dotw_prebooks` record: prebook_key (UUID), allocation_details, hotel_code, hotel_name, room_type, total_fare, total_tax, currency, is_refundable, expired_at (now + 3 min), resayil_message_id
- [x] **BLOCK-05**: Returns: prebook_key, hotel details, selected rates, countdown_timer_seconds (180, 179, ..., 0), expires_at timestamp
- [x] **BLOCK-06**: Rejects if allocation < 1 minute remaining (prompts re-search)
- [x] **BLOCK-07**: Logs to `dotw_audit_logs` with prebook_key created, allocation_expiry time
- [x] **BLOCK-08**: Only one active prebook per (company, WhatsApp user) allowed (new prebook expires previous)

### 7. Pre-Booking & Confirmation GraphQL

- [x] **BOOK-01**: GraphQL `createPreBooking` mutation accepts: prebook_key, passengers array (salutation, firstName, lastName, nationality, residenceCountry, email), resayil_message_id header
- [x] **BOOK-02**: Validates: all required passenger fields present, passenger count matches room configuration, email format valid
- [x] **BOOK-03**: Validates prebook_key exists and not expired (> 0 seconds remaining)
- [x] **BOOK-04**: Calls DOTW confirmBooking with passenger details, allocationDetails, email
- [x] **BOOK-05**: On success: Returns booking_confirmation_code, booking_status, itinerary_details
- [x] **BOOK-06**: Creates `dotw_bookings` record: confirmation_code, prebook_key, passengers, booking_status, hotel_details, resayil_message_id, company_id
- [x] **BOOK-07**: Logs to `dotw_audit_logs` with confirmation_code, booking_status
- [x] **BOOK-08**: On failure: Returns specific error (rate no longer available, hotel sold out, DOTW system error) with helpful message for N8N
...
- [x] **ERROR-01**: Invalid company credentials → 401 "DOTW credentials not configured for this company"
- [x] **ERROR-02**: DOTW API timeout (> 25 sec) → "Search taking too long, please try again" (N8N can retry)
- [x] **ERROR-03**: Allocation expired → "Rate offer expired, please search again" (clear, actionable)
- [x] **ERROR-04**: Rate no longer available → "This hotel/rate no longer available, similar options:" + suggest 3 alternatives
- [x] **ERROR-05**: Missing passenger field → "Please provide passenger {field_name}" (specific error per missing field)
- [x] **ERROR-06**: Hotel sold out → "Hotel full, showing 3 similar alternatives with availability"
- [x] **ERROR-07**: All errors logged to 'dotw' channel (no credentials, no full responses)
- [x] **ERROR-08**: Circuit breaker: If DOTW API fails 5x in 1 minute, return cached results (if available) or friendly "Try again in 30 seconds"

### 11. Modular Architecture

- [x] **MOD-01**: DOTW module lives in separate Laravel service (`app/Services/DotwService.php`)
- [x] **MOD-02**: DOTW config file (`config/dotw.php`) is environment-agnostic (no hardcoded paths)
- [x] **MOD-03**: DOTW migrations (`database/migrations/dotw_*.php`) are standalone and idempotent
- [x] **MOD-04**: DOTW GraphQL schema (`graphql/dotw.graphql`) is modular and composable
- [x] **MOD-05**: DOTW models (`app/Models/DotwPrebook.php`, `DotwBooking.php`, `DotwAuditLog.php`) use standard Eloquent patterns
- [x] **MOD-06**: No dependencies on invoice/task system (decoupled for v2 integration later)
- [x] **MOD-07**: Can copy entire DOTW module to production with: config changes + migrations + service + GraphQL schema
- [x] **MOD-08**: README documents deployment: env vars needed, migration steps, GraphQL schema registration
...
- [x] **B2B-05**: API designed for external B2B partners (N8N/Resayil is primary consumer, but extensible)
...
| B2B-05 | Phase 4 | Complete |
| BOOK-01 | Phase 9 | Complete |
| BOOK-02 | Phase 9 | Complete |
| BOOK-03 | Phase 9 | Complete |
| BOOK-04 | Phase 9 | Complete |
| BOOK-05 | Phase 9 | Complete |
| CANCEL-01 | Phase 9 | Complete |
| CANCEL-02 | Phase 9 | Complete |
| CANCEL-03 | Phase 9 | Complete |
| LOOKUP-01 | Phase 9 | Complete |
| LOOKUP-02 | Phase 9 | Complete |
| LOOKUP-03 | Phase 9 | Complete |
| LOOKUP-04 | Phase 9 | Complete |
| LOOKUP-05 | Phase 9 | Complete |
| LOOKUP-06 | Phase 9 | Complete |
| LOOKUP-07 | Phase 9 | Complete |
...
| CERT-08 | Phase 10 | Complete |
| CERT-09 | Phase 10 | Complete |
| CERT-10 | Phase 10 | Complete |
| CERT-11 | Phase 10 | Complete |
| CERT-12 | Phase 10 | Complete |
| CERT-13 | Phase 10 | Complete |
| CERT-14 | Phase 10 | Complete |
| CERT-15 | Phase 10 | Complete |
| CERT-16 | Phase 10 | Complete |
| CERT-17 | Phase 10 | Complete |
| CERT-18 | Phase 10 | Complete |
| CERT-19 | Phase 10 | Complete |
| CERT-20 | Phase 10 | Complete |
| REAL-01 | Phase 11 | Complete |
| REAL-02 | Phase 11 | Complete |
| REAL-03 | Phase 11 | Complete |
| REAL-04 | Phase 11 | Complete |
| REAL-05 | Phase 11 | Complete |
| COMPLY-01 | Phase 13 | Complete |
| COMPLY-02 | Phase 13 | Complete |
| COMPLY-03 | Phase 13 | Complete |
| COMPLY-04 | Phase 13 | Complete |
| COMPLY-05 | Phase 13 | Complete |
| COMPLY-06 | Phase 13 | Complete |
| COMPLY-07 | Phase 13 | Complete |
| COMPLY-08 | Phase 13 | Complete |

**Coverage:**
- Base v1 requirements: 54 (Phases 1-8) - All Complete
- Certification requirements: 43 (Phases 9-13) - All Complete
...
*DOTW V4 Certification Complete: 2026-03-10 (15 PASS / 5 SKIP / 0 FAIL)*
