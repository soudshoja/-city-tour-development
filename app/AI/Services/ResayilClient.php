...
class ResayilClient implements AIClientInterface
{
    ...
    public function __construct(?string $visionModel = null)
    {
        ...
        // Merge fixup (MERGE-PLAN-DEV-INTO-LAUNCH-2026-09-04.md, 4th auto-merge hazard):
        // this class is new from devsrv/port/phase1-launch-2026-08-24 and was auto-merged
        // in non-conflicting; config/ai.php's new 'primary' key defaults to 'resayil' when
        // AI_PRIMARY is unset, so this constructor throws unconditionally on ANY artisan
        // boot (route:list, migrate, tests — every command, not just AI use) on a checkout
        // whose .env predates these Resayil AI keys (e.g. RESAYIL_BASE/RESAYIL_API_KEY vs.
        // this .env's RESAYIL_BASE_URL/RESAYIL_API_TOKEN naming for the WhatsApp gateway).
        // Exempting 'local' alongside 'testing' matches this guard's own stated intent
        // ("fail fast on a genuinely broken PRODUCTION/staging deploy") without weakening
        // it there; a local machine without these credentials should boot with AI features
        // inert, not refuse to run any artisan command at all.
        $inUse = in_array('resayil', [config('ai.primary'), config('ai.fallback'), config('ai.default')], true);
        if ($inUse && ! in_array(config('app.env'), ['testing', 'local'], true) && (empty($this->apiUrl) || empty($this->apiKey))) {
