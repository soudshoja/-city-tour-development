...
/**
 * Resolver for the getServingCountries GraphQL query.
 *
 * Returns countries for which DOTW has hotel inventory via the getservingcountries command.
 * Use these codes to populate country selection in hotel search UI.
 *
 * LOOKUP-02
 */
class DotwGetServingCountries
{
    /**
     * Resolve the getServingCountries query.
     *
     * @param  mixed  $root  Unused GraphQL root value
     * @param  array  $args  GraphQL arguments (none required for this query)
     * @return array GetServingCountriesResponse shape
     */
    public function __invoke(mixed $root, array $args): array
    {
        ...
        try {
            ...
            $countries = $dotwService->getServingCountries();
            ...
            return [
                ...
                'error' => [
                    ...
                    'error_message' => 'Failed to fetch serving countries: ' . $e->getMessage(),
                    'error_details' => $e->getMessage(),
                    'action' => 'RETRY',
                ],
                'meta' => $this->buildMeta($companyId ?? 0),
