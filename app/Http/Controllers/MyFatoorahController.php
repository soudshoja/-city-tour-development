    ...
    public function callback()
    {
        try {
            ...
            if ($data->InvoiceStatus === 'Paid' && $data->focusTransaction->TransactionStatus === 'Succss') {
                ...
                // Client-facing redirect: invoice.show now requires a
                // logged-in, same-company staff session (see
                // InvoiceController's authorizeStaffInvoiceAccess()), and
                // the client's browser landing here after paying is not
                // authenticated staff -- use the signed, unauthenticated
                // '.public' variant instead. See Invoice::publicUrl().
                $redirectUrl = $invoice->publicUrl('show');
