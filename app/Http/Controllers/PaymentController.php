...
class PaymentController extends Controller
{
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    // Never flash raw exception text at the payer — it reads as a payment
                    // failure on a page that may already say PAID (VOU-2026-03849).
                    return redirect()->to($receiptInfo['url'])->with('error', 'We received your payment but could not finish processing it automatically. Please contact support if this page does not show PAID shortly.');
                }
    ...
    public function processMyFatoorahPaymentCompletion($payment, $statusData, $process, $partialId, $sendNotification = false)
    {
        ...
        try {
            // Webhook, browser callback and reconciler can all reach here for the same
            // payment within seconds; the loser of that race must exit as a no-op instead
            // of re-writing the row and erroring on the duplicate credit (VOU-2026-03849).
            // The lockForUpdate read serializes concurrent completions on the row.
