...
class VoucherDataRepository
{
    ...
    public function paymentState(Task $task, int $companyId): array
    {
        ...
        return $this->paymentStatePayload(
            $state,
            $invoice->status,
            // Merge fixup: unlike Task::issued_date, Invoice::paid_date carries no
            // 'datetime' cast (pre-existing on both merge parents, unmodified
            // here), so it comes back as a plain DB string, not a Carbon instance
            // — optional(string)->toDateString() silently returns null instead of
            // the date. Parse defensively rather than widening the model cast.
            $invoice->paid_date ? Carbon::parse($invoice->paid_date)->toDateString() : null,
            $linePaid
        );
    }
