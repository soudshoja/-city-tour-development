...
return new class extends Migration
{
    ...
    private function ensureIndexes(): void
    {
        Schema::table('coa_linkage_changes', function (Blueprint $table) {
            if (! Schema::hasIndex('coa_linkage_changes', 'coa_linkage_changes_run_id_index')) {
                $table->index('run_id', 'coa_linkage_changes_run_id_index');
            }

            if (! Schema::hasIndex('coa_linkage_changes', 'coa_linkage_changes_company_id_index')) {
                $table->index('company_id', 'coa_linkage_changes_company_id_index');
            }

            if (! Schema::hasIndex('coa_linkage_changes', 'coa_lc_run_company_idx')) {
                $table->index(['run_id', 'company_id'], 'coa_lc_run_company_idx');
            }

            if (! Schema::hasIndex('coa_linkage_changes', 'coa_lc_subject_idx')) {
                $table->index(['subject_table', 'subject_id'], 'coa_lc_subject_idx');
            }
        });
    }
