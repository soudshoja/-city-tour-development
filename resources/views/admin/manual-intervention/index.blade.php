...
<div class="container-fluid">
                        ...
                        <div class="table-responsive">
                            <table class="table table-sm">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td><code>{{ $stat->error_code }}</code></td>
                                        <td>{{ $stat->count }}</td>
                                        <td>
                                            @php
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            <form method="GET" action="{{ route('admin.manual-intervention.index') }}">
                <div class="row g-3">
                    <div class="col-md-3">
                        ...
                        <select name="error_code" class="form-select">
                            ...
                            @foreach($errorStats as $stat)
                                <option value="{{ $stat->error_code }}"
                                    {{ request('error_code') == $stat->error_code ? 'selected' : '' }}>
                                    {{ $stat->error_code }} ({{ $stat->count }})
                                </option>
                            @endforeach
                        </select>
                    </div>
                        ...
                        <select name="supplier_id" class="form-select">
                            ...
                            @foreach($suppliers as $supplierId)
                                <option value="{{ $supplierId }}"
                                    {{ request('supplier_id') == $supplierId ? 'selected' : '' }}>
                                    Supplier {{ $supplierId }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-2">
                        ...
                        <select name="document_type" class="form-select">
                            ...
                            <option value="air" {{ request('document_type') == 'air' ? 'selected' : '' }}>AIR</option>
                            <option value="pdf" {{ request('document_type') == 'pdf' ? 'selected' : '' }}>PDF</option>
                            <option value="image" {{ request('document_type') == 'image' ? 'selected' : '' }}>Image</option>
                            <option value="email" {{ request('document_type') == 'email' ? 'selected' : '' }}>Email</option>
                        </select>
                    </div>
                ...
                <div class="row mt-2">
                    <div class="col-12">
                        <a href="{{ route('admin.manual-intervention.index') }}" class="btn btn-secondary btn-sm">
                            Clear Filters
                        </a>
                    </div>
                </div>
    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h5 class="mb-0">Failed Documents ({{ $failedDocuments->total() }})</h5>
            <div>
                ...
                <a href="{{ route('admin.manual-intervention.export-csv', request()->all()) }}" class="btn btn-success btn-sm">
        ...
        <div class="card-body">
            @if($failedDocuments->isEmpty())
                <p class="text-muted">No failed documents found</p>
            @else
                <form id="bulk-retry-form" method="POST" action="{{ route('admin.manual-intervention.bulk-retry') }}">
                    @csrf
                    <div class="table-responsive">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <code style="font-size: 0.85em;">
                                            {{ Str::limit($doc->document_id, 16, '...') }}
                                        </code>
                                    </td>
                                    <td>{{ $doc->company->name ?? 'N/A' }}</td>
                                    <td><span class="badge bg-secondary">{{ $doc->supplier_id }}</span></td>
                                    <td>
                                        <span class="badge bg-info">{{ strtoupper($doc->document_type) }}</span>
                                    </td>
                                    <td>
                                        <code class="text-danger">{{ $doc->error_code }}</code>
                                    </td>
                                    <td>{{ Str::limit($doc->error_message, 50) }}</td>
                                    <td>
                                        <small>{{ $doc->created_at->diffForHumans() }}</small>
                                        <br>
                                        <small class="text-muted">{{ $doc->created_at->format('Y-m-d H:i') }}</small>
                                    </td>
                                    <td>
                                        <div class="btn-group" role="group">
                                            <a href="{{ route('admin.manual-intervention.show', $doc) }}"
                                               class="btn btn-sm btn-info">
                                                View
                                            </a>
                                            <a href="{{ route('admin.manual-intervention.timeline', $doc) }}"
                                               class="btn btn-sm btn-secondary">
                                                Timeline
                                            </a>
                                            <form method="POST"
                                                  action="{{ route('admin.manual-intervention.retry', $doc) }}"
