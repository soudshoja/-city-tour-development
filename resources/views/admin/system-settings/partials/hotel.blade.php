...
<script>
    function hotelsManager() {
        return {
            hotels: [],
            countries: @json(\App\Models\Country::where('is_active', 1)->orderBy('name')->get()),
            totalHotels: 0,
            totalPages: 1,
            currentPage: 1,
            perPage: 20,
            ...
            async loadHotels() {
                ...
                try {
                    ...
                    const response = await fetch(`{{ route('system-settings.hotels.list') }}?${params}`, {
            ...
            async saveCountry() {
                ...
                try {
                    const response = await fetch(`{{ route('system-settings.countries.store') }}`, {
                        method: 'POST',
                        headers: {
            ...
            async saveHotel() {
                this.saving = true;
                try {
                    const url = this.editingHotel ? `{{ route('system-settings.hotels.update', '') }}/${this.editingHotel.id}` : `{{ route('system-settings.hotels.store') }}`;
            ...
            async deleteHotel() {
                this.deleting = true;
                try {
                    const response = await fetch(`{{ route('system-settings.hotels.delete', '') }}/${this.deletingHotel.id}`, {
                        method: 'DELETE',
                        headers: {
                            'Accept': 'application/json',
                            'X-CSRF-TOKEN': '{{ csrf_token() }}'
