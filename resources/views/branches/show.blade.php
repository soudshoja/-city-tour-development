                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500">{{ $branch->agents->count() }} total agents</p>
                                </div>
                            </div>
                            <a href="{{ route('users.create', ['branch_id' => $branch->id, 'role' => 'agent']) }}"
                        ...
                        <div class="max-h-[400px] sm:max-h-[500px] overflow-y-auto">
                            <table class="w-full">
                                ...
                                <tbody class="divide-y divide-gray-100">
                                    @forelse ($branch->agents as $agent)
                                    <tr class="hover:bg-gray-50 transition">
                                        <td class="px-4 py-3">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div class="min-w-0">
                                                    <p class="text-sm font-medium text-gray-900 truncate agent-name">{{ $agent->name }}</p>
                                                    <p class="text-xs text-gray-500 md:hidden sm:block hidden">{{ $agent->phone_number ?? '—' }}</p>
                                                </div>
                                            </div>
                                        </td>
                ...
                <div class="bg-gradient-to-br from-[#4361ee] to-[#160f6b] rounded-lg shadow-sm overflow-hidden">
                    <div class="p-4 sm:p-5 border-b border-white/10">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3 min-w-0">
                                ...
                                <div class="min-w-0">
                                    <h3 class="text-white font-semibold text-base sm:text-lg truncate">{{ $branch->name }}</h3>
                                    <p class="text-white/70 text-xs sm:text-sm">{{ $branch->company->name ?? 'No Company' }}</p>
                                </div>
                            </div>
                    ...
                    <div class="p-4 sm:p-5 space-y-3 sm:space-y-4">
                        <div class="flex items-center justify-between">
                            ...
                            <span class="text-white text-sm font-medium">{{ $branch->gds_office_id ?? '—' }}</span>
                        </div>
                        <div class="flex items-center justify-between">
                            <span class="text-white/90 text-sm">Email</span>
                            <span class="text-white text-sm font-medium truncate ml-4 max-w-[180px]">{{ $branch->email ?? '—' }}</span>
                        </div>
                        <div class="flex items-center justify-between">
                            <span class="text-white/90 text-sm">Phone</span>
                            <span class="text-white text-sm font-medium">{{ $branch->phone ?? '—' }}</span>
                        </div>
                        <div class="pt-2 border-t border-white/10">
                            <span class="text-white/90 text-sm block mb-1">Address</span>
                            <span class="text-white text-sm leading-relaxed">{{ $branch->address ?? '—' }}</span>
                        </div>
                    </div>
                </div>
            </div>
