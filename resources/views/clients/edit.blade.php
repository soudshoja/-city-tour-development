    ...
    <div>
        ...
        <div class="bg-white rounded-md shadow-md p-4 m-2">
            <form action=" {{ route('clients.update', $client->id) }} " method="POST" class="w-full flex flex-col gap-2 justify-start items-center" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <input type="text" name="name" id="name" value="{{ $client->full_name }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring focus:ring-indigo-100 focus:border-indigo-300" placeholder="Name">
