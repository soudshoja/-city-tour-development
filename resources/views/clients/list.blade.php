<x-app-layout>
    ...
    <div class="mt-5 panel">
        ...
        <div class="flex mb-5">
          <p>Click <a href="{{ route('download.client') }}" class="text-primary">here</a> to download the Excel template</p>
        </div>
        <!-- Flex container for buttons and search input, with responsive handling for mobile -->
        <div class=" flex flex-col md:flex-row justify-between items-center w-full space-y-4 md:space-y-0">
            <!-- Buttons on the left -->
            <div class="flex space-x-2">
                    ...
                    <x-primary-button onclick="window.location='{{ route('clients.exportCsv') }}'">Export CSV</x-primary-button>
            </div>
    ...
    <table class="min-w-full mt-4">
        ...
        <tbody>
            @foreach ($clients as $client)
                <tr>
                <td  class="py-2 px-4 border">{{ $client->name }}</td>
                <td  class="py-2 px-4 border">{{ $client->email }}</td>
                <td  class="py-2 px-4 border">{{ $client->phone}}</td>
                <td  class="py-2 px-4 border">{{ $client->agent->name ?? 'N/A' }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
    ...
    document.getElementById('excelFileInput').addEventListener('change', function () {
        ...
        if (file) {
            ...
            fetch("{{ route('clientsupload.import') }}", {
                method: 'POST',
                headers: {
