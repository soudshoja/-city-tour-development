{{--
    Shared credit-ledger body, rendered from both:
      - resources/views/clients/credit.blade.php (authenticated staff view,
        wrapped in <x-app-layout>)
      - resources/views/clients/credit-shared.blade.php (public, signed-URL
        view, standalone HTML document -- no <x-app-layout> because that
        layout's menu assumes an authenticated auth()->user())

    Pulling this out keeps the authenticated page's markup byte-for-byte
    identical to before this fix (it just now goes through this @include),
    while letting the new public variant reuse the exact same statement
    content without pulling in app chrome that requires a logged-in user.

    $showPaginationLinks (bool, default true): the public/shared view passes
    false because Laravel's temporary signed URL only signs the exact query
    string it was generated with (id/expires/signature) -- x-pagination's
    links append a `page` param, which would no longer match that signature
    and would 403. The authenticated view keeps real pagination links.
--}}
        ...
        <div class="bg-white dark:bg-slate-900/60 rounded-2xl shadow-lg overflow-hidden border border-gray-200 dark:border-slate-700">
            ...
            @if($credits->count() > 0)
                ...
                <div class="px-6 py-4 border-t border-gray-200 dark:border-slate-700">
                    @if($showPaginationLinks ?? true)
                        <x-pagination :data="$credits" />
                    @else
                        <p class="text-sm text-gray-500 dark:text-slate-400 text-center">
                            Showing {{ $credits->count() }} of {{ $credits->total() }} transaction(s).
                            @if($credits->total() > $credits->count())
                                Ask for an updated link to see the rest.
                            @endif
                        </p>
                    @endif
                </div>
