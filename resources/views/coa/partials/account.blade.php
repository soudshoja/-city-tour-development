<li class="border-l-2 border-gray-300 hover:bg-gray-100 account-item" data-id="{{ $account->id }}"
    data-name="{{ $account->name }}" :class="{
        'bg-blue-100 my-5': '{{ $account->name }}' === 'Assets',
        'bg-green-100 my-5': '{{ $account->name }}' === 'Liabilities',
        'bg-yellow-100 my-5': '{{ $account->name }}' === 'Income',
        'bg-red-100 my-5': '{{ $account->name }}' === 'Expenses'
    }">
