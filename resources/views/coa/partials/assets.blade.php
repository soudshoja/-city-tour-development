          ...
          <ul class="w-full">
              ...
              <li x-data="{ showAddCategoryForm: false }" class="level2 relative w-full flex items-center">
                  <a href="javascript:;"
                      ...
                      :class="{'border-l-4 border-[#508D4E] text-[#000000]': openLevels['{{ $asset->id }}']}"
                      @click="openLevels = { ['{{ $asset->id }}']: !openLevels['{{ $asset->id }}'] }">

                      <span class="flex-1">{{ $asset->name }}</span>
                      ...
                      <div class="flex items-center gap-2">
                          ...
                          <!-- "+" Icon Button -->
                          <button @click.stop="showAddCategoryForm = !showAddCategoryForm" class="text-green-600 hover:text-green-800">
              ...
              <div x-show="openLevels['{{ $asset->id }}']"
                  ...
                  <div x-data="{ showAddCategoryForm: false }" class="level3 relative w-full flex items-center">
                      <a href="javascript:;"
                          class="flex items-center px-4 py-2 w-full hover:text-[#508D4E] transition-all gap-x-2"
                          :class="{'border-l-4 border-[#80AF81] text-[#000000]': openLevels['{{ $asset->id }}']}"
                          @click="openLevels['{{ $level3asset->id }}'] = !openLevels['{{ $level3asset->id }}']">

                          <span class="flex-1">{{ $level3asset->name }}</span>
                          <div class="flex items-center gap-2">
                  ...
                  <div x-data="{
                            newCategoryName: '',
                            newCategoryCode: '',
                            newCategoryLevel: {{ $level3asset->level }},
                            newCategoryParentId: {{ $level3asset->id }},
                            variance: 0,
                            budgetBalance: 0,
                            actualBalance: 0,
                        }"
              ...
              <div x-show="openLevels['{{ $level3asset->id }}']" class="ml-6 space-y-2">
                  ...
                  <div
                      ...
                      class="level4 flex items-center justify-between p-4 rounded-lg shadow-sm w-full group cursor-pointer">
                      <span class="text-gray-800 dark:text-gray-600 font-medium group-hover:text-green-600">{{ $level4asset->name }}</span>
                  ...
                  <div x-show="openLevels['{{ $level4asset->id }}']" class="ml-6 space-y-2">
                      ...
                      <div class="level5 flex items-center justify-between p-4 rounded-lg shadow-sm  w-full">
                          ...
                          <input type="text" name="code"
                              class="text-center border-none focus:outline-none focus:ring-0 px-2 py-1 text-xs font-semibold text-green-600 bg-green-100 rounded-full editable-cell"
                              value="{{ $level5asset->code }}" id="editable-code-{{ $level5asset->id }}"
                              onblur="saveCode({{ $level5asset->id }}, this.value)">
                          <div>
                              <input type="text" name="actual_balance"
                                  class="text-center border-none focus:outline-none focus:ring-0 px-2 py-1 text-xs font-semibold text-green-600 rounded-full {{ $level4asset->actual_balance < 0 ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600' }}"
                                  value="{{ number_format($level5asset->actual_balance, 2) }}" readonly
                                  style="pointer-events: none; cursor: default;">
                          </div>
                      </div>
                      @endforeach
