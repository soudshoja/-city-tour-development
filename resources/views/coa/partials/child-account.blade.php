<li x-data="{ open: false, showAddCategoryForm: false }" class="relative w-full flex flex-col">
    <div class="group grid grid-cols-12 gap-2 items-center py-3 px-4 border-b border-gray-200 dark:border-gray-700 hover:bg-gradient-to-r hover:from-{{ $color }}-50/50
        ...
        <div class="col-span-5 flex items-center gap-2 pl-1">
            @if ($account->is_group || ($account->childAccounts && $account->childAccounts->isNotEmpty()))
                <svg class="w-4 h-4 text-gray-400 group-hover:text-{{ $color }}-500 transition-all duration-200 flex-shrink-0"
                    :class="{ 'rotate-90 text-{{ $color }}-500': open }" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            @if ($account->ledger)
                <a class="text-xs text-blue-500 hover:text-blue-700 hover:underline flex-shrink-0 transition-colors duration-200"
                    target="_blank" href="{{ route('journal-entries.show', $account->id) }}" @click.stop>
                    Ledger
                </a>
            @endif
        </div>
        ...
        <div class="col-span-2 flex justify-center">
            <span class="px-3 py-1 text-xs font-semibold rounded-full bg-{{ $color }}-100 text-{{ $color }}-600 group-hover:bg-{{ $color }}-200 transition-colors duration-200">
                {{ $account->code }}
            </span>
        </div>

        <div class="col-span-3 flex items-center justify-end gap-2">
            <span class="px-3 py-1 text-xs font-semibold rounded-full {{ $account->balance < 0 ? 'bg-red-100 text-red-600' : 'bg-' . $color . '-100 text-' . $color . '-600' }}
            ...
            @if(isset($account->excluded_payment_balance) && $account->excluded_payment_balance != 0)
                <span class="px-2 py-1 text-xs rounded bg-yellow-100 text-yellow-700 border border-yellow-300 flex-shrink-0"
                    title="Payment accounts excluded: {{ number_format($account->excluded_payment_debit - $account->excluded_payment_credit, 2) }}">
                    ※ Excl: {{ $account->excluded_payment_balance }}
                </span>
            @endif
        ...
        <div class="col-span-2 flex items-center justify-end gap-2">
            ...
            <div x-data="{ delegateBalanceAmadeus: false }">
                ...
                <div x-cloak x-show="delegateBalanceAmadeus"
                    ...
                    <div @click.away="delegateBalanceAmadeus = false"
                        ...
                        class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-lg mx-4 p-6 relative border-t-4 border-{{ $color }}-500">
                        ...
                        <h2 class="text-xl font-semibold mb-3 text-{{ $color }}-600">Delegate Balance</h2>
                        ...
                        <form action="{{ route('coa.delegate-price') }}" method="POST">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="number" name="code" required placeholder="Enter new code" min="{{ $account->code + 1 }}"
                                    class="w-full border border-gray-300 dark:border-gray-600 dark:bg-gray-700 rounded-lg text-sm px-3 py-2 
                                           focus:outline-none focus:ring-2 focus:ring-{{ $color }}-500 focus:border-transparent transition-all duration-200">
                            </div>
                            <div class="flex justify-end gap-3">
                                ...
                                <button type="submit"
                                    class="bg-{{ $color }}-600 text-white px-4 py-2 rounded-lg hover:bg-{{ $color }}-700
                                           shadow-lg shadow-{{ $color }}-500/30 hover:shadow-{{ $color }}-500/50 transition-all duration-200">
                                    Delegate
                                </button>
                            </div>
                        </form>
            ...
            @if ($account->is_group)
                <button @click.stop="showAddCategoryForm = true"
                    class="p-1.5 text-{{ $color }}-500 hover:text-{{ $color }}-700 hover:bg-{{ $color }}-50 rounded-lg transition-all duration-200 opacity-0 group-hover:opacity-100"
    ...
    <div x-show="showAddCategoryForm" x-cloak
        ...
        <div @click.away="showAddCategoryForm = false"
            ...
            <h2 class="text-xl font-semibold mb-3 text-{{ $color }}-600">New Account</h2>
            <hr class="mb-3 border-gray-200 dark:border-gray-700">
            <form action="{{ route('coa.addCategory') }}" method="POST">
                ...
                <div class="space-y-4">
                    ...
                    <div id="entity-container-{{ $account->id }}"></div>
                </div>
                ...
                <div class="flex justify-end gap-3 mt-6">
                    ...
                    <button type="submit" class="bg-{{ $color }}-600 text-white px-4 py-2 rounded-lg hover:bg-{{ $color }}-700
                        shadow-lg shadow-{{ $color }}-500/30 hover:shadow-{{ $color }}-500/50 transition-all duration-200">
                        Create New
                    </button>
                </div>
    ...
    <ul x-show="open"
        ...
        x-transition:enter-start="opacity-0"
        x-transition:enter-end="opacity-100"
        class="ml-6 border-l-2 border-{{ $color }}-200 dark:border-{{ $color }}-800/50">
        @if ($account->childAccounts && $account->childAccounts->isNotEmpty())
            @foreach ($account->childAccounts as $childAccount)
                @include('coa.partials.child-account', ['account' => $childAccount, 'color' => $color])
            @endforeach
        @else
