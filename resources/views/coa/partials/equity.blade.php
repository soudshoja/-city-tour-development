  <div
      ...
      <div class="flex items-center space-x-3 ">
          ...
          <h3 class="font-semibold text-lg text-[#ffc107]">equity</h3>
      </div>
      <!-- Status Badge -->
...
<!-- equity Section overview -->
<div id="equityDetails" class="rounded-lg shadow-sm" x-data="equityManager()">
    <div class="mb-5">
        <!-- Vertical layout for top-level equity -->
        <ul class="space-y-2 w-full">
            <!-- Level 2 - Top-Level equity as Tabs -->
            ...
            <li x-data="{ showAddCategoryForm: false }" class="level2 relative w-full flex items-center">
                <a href="javascript:;"
                    ...
                    :class="{'border-l-4 border-[#ffc107] text-[#000000]': openLevels['{{ $equity->id }}']}"
                    @click="openLevels['{{ $equity->id }}'] = !openLevels['{{ $equity->id }}']">

                    <span class="flex-1">{{ $equity->name }}</span>

                    <div class="flex items-center gap-2">

                    <svg x-show="!openLevels['{{ $equity->id }}']" width="24" height="24" viewBox="0 0 24 24"
                ...
                <div x-show="showAddCategoryForm"
                      ...
                      <div class="bg-white rounded-xl shadow-lg w-full max-w-lg mx-4 p-6 relative">
                          ...
                          <h2 class="text-xl font-semibold mb-3">New equity</h2>
                          <hr class="mb-3">
            ...
            <!-- Level 3 - Nested content opens below each top-level equity when clicked -->
            <div x-show="openLevels['{{ $equity->id }}']" class="mt-2 space-y-2 p-4 w-full">
               
                @if ($equity->level3equity->isEmpty())
                <p class="text-danger">No equity here yet!</p>
                @endif
                ...
                <div x-data="{ showAddCategoryForm: false }" class="level3 relative w-full flex items-center">
                    <a href="javascript:;"
                        ...
                        :class="{'border-l-4 border-[#ffc10780] text-[#000000]': openLevels['{{ $level3equity->id }}']}"
                        @click="openLevels['{{ $level3equity->id }}'] = !openLevels['{{ $level3equity->id }}']">
                        
                        <!-- Text on the left -->
                        <span class="flex-1">{{ $level3equity->name }}</span>
                        <div class="flex items-center gap-2">

                        <!-- Icons -->
                        <svg x-show="!openLevels['{{ $level3equity->id }}']" width="24" height="24"
                    ...
                    <div x-data="{
                            newCategoryName: '',
                            newCategoryCode: '',
                            newCategoryLevel: {{ $level3equity->level }},
                            newCategoryParentId: {{ $level3equity->id }},
                            variance: 0,
                            budgetBalance: 0,
                            actualBalance: 0,
                        }"
                ...
                <!-- Level 4 - Nested under each level 3 equity -->
                <div x-show="openLevels['{{ $level3equity->id }}']" class="ml-6 space-y-2 mt-2">
                    @if ($level3equity->level4equity->isEmpty())
                    <p class="text-danger">No equity here yet!</p>
                    @endif
                    ...
                    <div class="level4 flex items-center justify-between p-4 rounded-lg shadow-sm w-full">
                        <span class="text-gray-800 dark:text-gray-600 font-medium">{{ $level4equity->name }}</span>
                        ...
                        <input type="text" name="code"
                            ...
                            value="{{ $level4equity->code }}" id="editable-code-{{ $level4equity->id }}"
                            onblur="saveCode({{ $level4equity->id }}, this.value)"
                            onkeypress="checkEnter(event, {{ $level4equity->id }}, this.value)">

                        <!-- Actual Balance -->
                        <div @click="window.location='{{ route('coa.transaction') }}?level4Id={{ $level4equity->id }}'">
                            <input type="text" name="actual_balance"
                                ...
                                value="{{ number_format($level4equity->actual_balance, 2) }}" readonly
  ...
  <script>
      document.addEventListener('alpine:init', () => {
          Alpine.data('equityManager', () => ({
              ...
              openLevels: {},
              equity: @json($equity), // Initialize the equity array with the existing equity
              addCategory() {
                      ...
                      .then(data => {
                          if (data.success) {
                              ...
                              this.equity.push({
                                  ...
                                  parent_id: this.newCategoryParentId,
                                  variance: this.variance,
                                  level3assets: [] // Initialize with an empty array for level 3 equity
                              });
                              this.newCategoryName;
                              this.newCategoryCode;
  ...
  <script>
      // open and close the equity section details function
    ...
    //   console.log(equityToggleButton);

      // Initially hide the content div
      contentequityDiv.style.display = 'none';

      // Add click event listener to the button
      equityToggleButton.forEach(function(button) {
        //   console.log(button);
          button.addEventListener('click', function() {
              // Toggle the content div visibility
              if (contentequityDiv.style.display === 'none' || contentequityDiv.style.display ===
                  '') {
                  contentequityDiv.style.display = 'block'; // Show the content
              } else {
                  contentequityDiv.style.display = 'none'; // Hide the content
              }
          });
      });


      // update the equity code function

      function checkEnter(event, equityId, value) {
          if (event.key === 'Enter') {
              event.preventDefault(); // Prevent form submission if it's in a form
              saveequityCode(equityId, value);
          }
      }

      function saveequityCode(equityId, value) {
          if (value.trim() === '') {
          ...
          fetch(`/updateCode/${equityId}`, { // Update with your save route
                  method: 'POST',
                  headers: {
                      'Content-Type': 'application/json',
