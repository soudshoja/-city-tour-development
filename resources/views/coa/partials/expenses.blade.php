...
<div class="main-container flex w-full rounded-lg shadow-sm border border-gray-200">
    <!-- Left Div (90%) -->
    <div class="left-div ExpensesToggleButton cursor-pointer flex items-center justify-between bg-white p-4">
    ...
    <div class="right-div flex justify-center items-center p-4">
        ...
        <button id="refreshExpensesButton">
            <svg class="ExpensesRefreshIcon" width="24" height="24" viewBox="0 0 24 24" fill="none"
...
<script>
    ...
    ExpensesToggleButton.forEach(button => {
        button.addEventListener('click', () => {
            contentExpensesDiv.style.display = (contentExpensesDiv.style.display === 'none') ? 'block' : 'none';
        });
    });

    // Refresh Expenses Data
    document.getElementById("refreshExpensesButton").addEventListener("click", function () {
        const svgIcon = document.querySelector(".refresh-icon");
        svgIcon.classList.toggle("rotate");

        setTimeout(() => {
            refreshExpensesData(); // Replace with your data fetching logic
        }, 300);
    });

    function refreshExpensesData() {
        console.log("Fetching new expenses data...");
        // Fetch new data
    }
