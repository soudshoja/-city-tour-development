                    ...
                    <div x-cloak x-show="showFilter" x-transition
                        ...
                        <form method="GET" action="{{ route('coa.transaction') }}" class="flex flex-col space-y-4">
                            ...
                            <x-multi-picker 
                                label="Agents"
                                name="agent_ids"
                                :items="$agents->map(fn($a) => ['id' => $a->id, 'name' => $a->name])->values()->all()"
                                :preselected="collect(request('agent_ids', []))->map(fn($v) => (int)$v)->all()"
                                allLabel="All agents"
                            ...
                            <x-multi-picker 
                                label="Accounts"
                                name="account_ids"
                                :items="$accounts->map(fn($a) => ['id' => $a->id, 'name' => $a->name])->values()->all()"
                                :preselected="collect(request('account_ids', []))->map(fn($v) => (int)$v)->all()"
