<div x-data="sidebarCompanySelector()" class="flex flex-col items-center relative">
    ...
    <div x-show="open" x-cloak 
        ...
        class="absolute left-full ml-3 top-0 w-64 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 z-50 overflow-hidden">
        ...
        <div class="max-h-60 overflow-y-auto">
            ...
            <button type="button"
                x-show="'{{ strtolower($company->name) }}'.includes(search.toLowerCase())"
                @click="selectCompany({{ $company->id }}, '{{ addslashes($company->name) }}')"
                :disabled="loading"
                class="w-full px-4 py-3 text-left text-sm hover:bg-blue-50 dark:hover:bg-gray-700 flex items-center gap-3 transition-colors disabled:opacity-50 {{ $company->id == $currentCompanyId ? 'bg-blue-50 dark:bg-gray-700 border-l-4 border-blue-500' : 'text-gray-700 dark:text-gray-300' }}">
                ...
                <span class="font-medium text-blue-600 dark:text-blue-400">{{ $company->name }}</span>
                @else
...
<script>
    function sidebarCompanySelector() {
        return {
            ...
            search: '',
            loading: false,
            selectedCompanyId: {{ $currentCompanyId ?? 1 }},
            ...
            async selectCompany(id, name) {
                ...
                try {
                    const response = await fetch('{{ route("users.set-company") }}', {
                        method: 'POST',
                        headers: {
