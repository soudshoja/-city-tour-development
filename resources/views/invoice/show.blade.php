...
<head>
  ...
  <style>
    input[type="checkbox"].disabled-checkbox {
      ...
      background-color: #e2e8f0;  /* Light gray background to show it's disabled */
    }

    tr.disabled-row {
      cursor: not-allowed;  /* Change cursor to indicate the row is not clickable */
      opacity: 0.6;  /* Reduce opacity to indicate it's disabled */
      background-color: #e2e8f0;  /* Light gray background to show the row is disabled */
    }
    ...
    tr.disabled-row input[type="checkbox"] {
      ...
      opacity: 1;  /* Keep checkbox opacity full */
    }
    ...
    <table class="min-w-full mb-8 border border-gray-200">
      ...
      <tbody>
        ...
        <tr class="text-sm text-gray-700">
          <td class="px-4 py-2 border">{{ $detail->task_description ?? 'N/A' }} (Note: {{ $detail->client_notes ?? 'N/A' }})</td>
      ...
      <table class="min-w-full mb-8 border border-gray-200">
        ...
        <tbody>
          ...
          <td class="px-4 py-2 border">{{ $partial->payment->voucher_number ?? 'N/A'}}</td>
            ...
            <td class="px-4 py-2 border">{{ $partial->payment ? \Carbon\Carbon::parse($partial->payment->payment_date)->format('d M, Y H:i') : 'N/A' }}</td>
