...
<head>
    ...
    <title>Uninvoiced Task Reminder</title>
    @if($isPdf ?? false)
    <style>
        @page {
            margin: 25px;
                                ...
                                <tr>
                                    ...
                                    <td width="50%" valign="top" align="right">
                                        ...
                                        <table role="presentation" cellspacing="0" cellpadding="0" border="0" style="margin-top:{{ ($isPdf ?? false) ? '10px' : '15px' }};margin-left:auto;">
                                            ...
                                            <tr>
                                                <td style="padding:{{ ($isPdf ?? false) ? '3px 10px 3px 0' : '4px 15px 4px 0' }};font-size:{{ ($isPdf ?? false) ? '10px' : '13px' }};color:#666;text-align:right;">Agent:</td>
                                                <td style="padding:{{ ($isPdf ?? false) ? '3px 0' : '4px 0' }};font-size:{{ ($isPdf ?? false) ? '10px' : '13px' }};font-weight:bold;color:#333;">{{ $agent->name ?? 'Not Set' }}</td>
                                            </tr>
                                            <tr>
                            ...
                            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="background-color:#eff6ff;border-left:4px solid #3b82f6;{{ ($isPdf ?? false) ? '' : 'border-radius:4px;' }}">
                                <tr>
                                    <td style="padding:{{ ($isPdf ?? false) ? '10px 15px' : '15px 20px' }};">
                                        <p style="margin:0;font-size:{{ ($isPdf ?? false) ? '12px' : '15px' }};font-weight:bold;color:#1e40af;">Uninvoiced Task Reminder</p>
                                        <p style="margin:5px 0 0 0;font-size:{{ ($isPdf ?? false) ? '10px' : '13px' }};color:#1e40af;">Dear {{ $agent->name ?? 'Agent' }}, you have {{ count($tasks) }} task(s) that have not been invoiced yet. Please create invoices for the following tasks as soon as possible.</p>
                                    </td>
                                </tr>
                            </table>
                        </td>
                        ...
                        <td style="padding:{{ ($isPdf ?? false) ? '3px 25px 10px' : '5px 40px 15px' }};">
                            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="border:1px solid #e0e0e0;{{ ($isPdf ?? false) ? '' : 'border-radius:4px;overflow:hidden;' }}">
                                <tr>
                                    ...
                                    <th style="background-color:#004c9e;padding:{{ ($isPdf ?? false) ? '8px 10px' : '12px 15px' }};font-size:{{ ($isPdf ?? false) ? '9px' : '12px' }};font-weight:bold;color:#fff;text-align:left;text-transform:uppercase;">GDS Reference</th>
                                    <th style="background-color:#004c9e;padding:{{ ($isPdf ?? false) ? '8px 10px' : '12px 15px' }};font-size:{{ ($isPdf ?? false) ? '9px' : '12px' }};font-weight:bold;color:#fff;text-align:left;text-transform:uppercase;">Airline Reference</th>
                                ...
                                <tr style="background-color:{{ $bgColor }};">
                                    ...
                                    <td style="padding:{{ ($isPdf ?? false) ? '8px 10px' : '12px 15px' }};font-size:{{ ($isPdf ?? false) ? '10px' : '13px' }};color:#333;border-bottom:1px solid #e0e0e0;">{{ $task->gds_reference ?? 'N/A' }}</td>
                                    <td style="padding:{{ ($isPdf ?? false) ? '8px 10px' : '12px 15px' }};font-size:{{ ($isPdf ?? false) ? '10px' : '13px' }};color:#333;border-bottom:1px solid #e0e0e0;">{{ $task->airline_reference ?? 'N/A' }}</td>
                                ...
                                <tr style="background-color:#f0f4ff;">
                                    <td colspan="8" style="padding:{{ ($isPdf ?? false) ? '8px 10px' : '12px 15px' }};font-size:{{ ($isPdf ?? false) ? '10px' : '13px' }};font-weight:bold;color:#004c9e;text-align:right;">Total:</td>
