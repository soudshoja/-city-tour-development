        ...
        <table class="min-w-full bg-white border border-gray-200 rounded shadow">
            ...
            <tbody>
                ...
                <tr class="border-b hover:bg-gray-50">
                    ...
                    <td class="px-4 py-2" title="{{ $payment->payment_reference ? ($payment->invoice_ref ? $payment->payment_reference . '/' . $payment->invoice_ref : $payment->payment_reference) : 'N/A' }}">
                        {{ \Illuminate\Support\Str::limit($payment->payment_reference ? ($payment->invoice_ref ? $payment->payment_reference . '/' . $payment->invoice_ref : $payment->payment_reference) : 'N/A', 15) }}
                    </td>
                    ...
                    <td class="px-4 py-2 relative">
                        <div x-data="{ editPaymentLink: false }">
                            <button @click="editPaymentLink = true" class="flex items-center gap-2 p-2 border-2 border-transparent rounded-full hover:border-blue-500 hover:bg-blue-50 transition" data-tooltip-left="Edit">
                                ...
                                <div class="bg-white p-6 rounded shadow-lg w-full max-w-md relative">
                                    ...
                                    <form action="{{ route('payment.link.update', $payment->id) }}" method="POST">
                                        ...
                                        <div class="flex justify-between space-x-4">
                                            <button type="button" @click="editPaymentLink = false" class="rounded-full shadow-md border border-gray-200 hover:bg-red-300 px-4 py-2">Cancel</button>
                        ...
                        <form action="" method="POST" class="inline-block">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="flex items-center justify-center p-2 border-2 border-transparent rounded-full hover:border-red-500 hover:bg-red-50 transition" data-tooltip-left="Delete">
