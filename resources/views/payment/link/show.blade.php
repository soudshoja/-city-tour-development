...
<body class="overflow-y-auto font-nunito antialiased bg-gray-100 py-10">
    <div class="max-w-4xl mx-auto p-8 bg-white shadow-lg rounded-lg">
        ...
        <div class="flex justify-between items-start mb-8">
            <div>
                <h1 class="text-3xl font-bold text-gray-800">Payment Voucher</h1>
                <p class="text-sm text-gray-600">Voucher #: {{ $payment->voucher_number }}</p>
                <p class="text-sm text-gray-600">Date: {{ $payment->created_at->format('d M, Y') }}</p>
            </div>
            <div class="text-right">
                <img src="{{ $companyLogoSrc }}" alt="Company Logo" class="h-16 w-auto mb-2 inline-block">
                <h2 class="text-xl font-bold text-gray-800">{{ $payment->agent->branch->company->name }}</h2>
                <p class="text-sm text-gray-600">{{ $payment->agent->branch->company->address }}</p>
                <p class="text-sm text-gray-600">{{ $payment->agent->branch->company->phone }}</p>
                <p class="text-sm text-gray-600">{{ $payment->agent->branch->company->email }}</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-8 mb-8">
            ...
            <div class="text-right">
                <h3 class="text-lg font-semibold text-gray-800">Notes from <span class="font-semibold"> Agent: {{ $payment->agent->name }}</span></h3>
                <p class="text-sm text-gray-600 mt-2">{{ $payment->notes ?? 'No notes provided.' }}</p>
            </div>
        </div>
        ...
        <div class="mb-5">
            @unless ($payment->status === 'completed')
                <form action="{{ route('payment.link.initiate') }}" method="POST" class="flex items-center gap-4">
                    ...
                    <button type="submit"
                        class="city-light-yellow hover:text-white hover:bg-[#004c9e] rounded-full border border-gray-300 px-4 py-2 font-semibold">
                        Pay Now
                    </button>
        ...
        <div class="flex justify-between items-end mt-10">
            <div class="text-sm text-gray-600">
                <p>If you have any questions about this voucher, please contact:</p>
                <p>
                    {{ $payment->agent->name }} - {{ $payment->agent->email }}
                    @if ($payment->agent->phone)
                        || {{ $payment->agent->phone }}
                    @endif
                </p>
            </div>
            <p class="text-right font-bold text-gray-800">Thank you for your business!</p>
        </div>
    </div>
</body>
