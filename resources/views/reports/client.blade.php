...
<div class="container mx-auto p-6">
    <h1 class="text-2xl font-bold text-gray-800 mb-6">Client Accounting Report</h1>
    ...
    <div class="overflow-x-auto bg-white rounded-lg shadow-md">
        <table class="min-w-full bg-white border border-gray-200">
            <thead>
                <tr>
                    <th class="py-2 px-4 bg-gray-100 text-gray-700 font-semibold text-left border-b border-gray-200">Client Name</th>
                    ...
                    <th class="py-2 px-4 bg-gray-100 text-gray-700 font-semibold text-left border-b border-gray-200">Total Completed</th>
                    <th class="py-2 px-4 bg-gray-100 text-gray-700 font-semibold text-left border-b border-gray-200">Total Pending</th>
                </tr>
            </thead>
            <tbody>
                @foreach($clients as $client)
                <tr class="hover:bg-gray-50">
                    <td class="py-2 px-4 border-b">{{ $client->client_name }}</td>
                    <td class="py-2 px-4 border-b">{{ $client->total_transactions }}</td>
                    <td class="py-2 px-4 border-b">{{ $client->total_debit }}</td>
                    <td class="py-2 px-4 border-b">{{ $client->total_credit }}</td>
                    <td class="py-2 px-4 border-b">{{ $client->total_completed }}</td>
                    <td class="py-2 px-4 border-b">{{ $client->total_pending }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
    ...
    <!-- Client Ledger Details Table -->
    <h2 class="text-xl font-bold text-gray-800 mt-10 mb-6">Client Ledger Details</h2>
    <div class="overflow-x-auto bg-white rounded-lg shadow-md">
        <table class="min-w-full bg-white border border-gray-200">
            <thead>
                <tr>
            ...
            <tbody>
                @foreach($clientLedgers as $ledger)
                <tr class="hover:bg-gray-50">
                    <td class="py-2 px-4 border-b">{{ $ledger->client_name }}</td>
