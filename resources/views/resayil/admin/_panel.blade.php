{{--
    ...
    This is the CONTENT of the section only — no <x-app-layout> here. It is
    included from two places:
      1. resayil/admin/index.blade.php (standalone route, resayil-admin.index)
         — still a real URL, still bookmarkable, still works if someone lands
         on it directly.
      2. settings/index.blade.php, as the "WhatsApp" tab — the redesign's
         main point: this panel now lives INSIDE Settings instead of being a
         page that navigates away from it. Pass `embedded: true` from there.

    Expected variables (all provided by ResayilAdminController::index(), or
    forwarded by SettingController::index() when embedded):
      $overview, $companyId, $isOperator, $activePanel, $embedUrl,
      $notConfigured, $embedded (optional, default false)
    ...
    a css file. Plain CSS values (not Tailwind classes) are unaffected by
    the arbitrary-value limitation, so normal CSS like `height: 70vh` is
    safe to use inside this block.
...
{{--
    Pushed unconditionally — BOTH entry points need every rule below. The
    standalone route wraps this in its own <x-app-layout> and the settings
    tab wraps it in settings/index.blade.php's <x-app-layout>; either way
    the layout's own @stack('styles') in <head> is what actually renders
    this. Blade stacks accept multiple @push calls to the same name (this
    partial's own plus settings/index.blade.php's Vite @push are both on
    the stack at once), so pushing here a second time on the same request
    is normal, not a conflict — the earlier version of this file wrongly
    skipped this whole block when embedded, which shipped a completely
    unstyled panel inside Settings (a real bug, caught by screenshotting
    the embedded tab during verification, not by inspection).
--}}
@push('styles')
    <style>
        ...
        /*
            PRE-EXISTING APP-SHELL BUG, fixed here scoped to this page only.

            <html> and <body> both carry a fixed viewport height with their
            own `overflow-y: auto`. Per the CSS Overflow spec, the browser
            only lets <body>'s scrollbox drive normal page scrolling
            (mouse wheel, trackpad, PageDown, the browser's own scrollbar)
            when <html>'s overflow is the INITIAL value `visible` — the
            moment <html> gets an explicit overflow of its own (as it does
            here), <html> becomes the thing the browser scrolls, and
            <body>'s real overflow is stranded one level down, reachable
            only by directly setting `body.scrollTop` from script.

            Verified live: with `<html class="resayil-reflow">` sized to
            exactly the viewport (889px here) and <body> containing 1473px
            of real content, `document.documentElement.scrollHeight ===
            document.documentElement.clientHeight` (889 === 889) — the
            browser believes there is nothing to scroll, so any content
            past the first screenful (the Team seats card, the Setup
            checklist, and now the Team and Inbox tabs below) is fully
            present in the DOM and passes an innerText check, but is
            unreachable by every normal scroll gesture and invisible in a
            full-page screenshot. That is the "seat card renders but is
            visually hidden" symptom.

            This is a page-INDEPENDENT app-shell issue (confirmed live: it
            has nothing to do with the Resayil drawer's own reflow class,
            which only sets a margin), not something introduced by this
            panel — this panel is simply one of the few pages tall enough
            to hit it. The real fix belongs in the shared layout/compiled
            CSS, which this project cannot touch without `npm run build`.
            Restoring <html>'s overflow to its initial `visible` value here
            re-enables the browser's normal body-drives-the-viewport
            behaviour for THIS page only (a bare <style> tag is
            document-global, not scoped to this file's markup, but it only
            exists in the document when this view is loaded) — no shared
            layout file is touched and no rebuild is required.
        */
        ...
        .rsa-btn-link { border: none; background: none; padding: 0; color: #2563eb; font-size: .75rem; font-weight: 600; cursor: pointer; }
        .rsa-btn-link:hover { text-decoration: underline; }
        .dark .rsa-btn-link { color: #93c5fd; }

        .rsa-tabs { display: flex; gap: .25rem; border-bottom: 1px solid #e5e7eb; margin-bottom: 1.25rem; overflow-x: auto; }
        .dark .rsa-tabs { border-color: #374151; }
        .rsa-tab { padding: .6rem .95rem; font-size: .875rem; font-weight: 500; color: #6b7280; background: none; border: none; border-bottom: 2px solid transparent; cursor: pointer; white-space: nowrap; transition: color .15s ease, border-color .15s ease; }
        ...
        .rsa-panel { animation: rsa-fade .18s ease; }
        @keyframes rsa-fade { from { opacity: 0; transform: translateY(2px); } to { opacity: 1; transform: translateY(0); } }
        @media (prefers-reduced-motion: reduce) { .rsa-panel { animation: none; } }

        .rsa-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 1rem; }
        .rsa-card { background: #fff; border: 1px solid #e5e7eb; border-radius: .75rem; padding: 1.1rem 1.15rem; transition: border-color .15s ease; }
        .rsa-card:hover { border-color: #d1d5db; }
        .dark .rsa-card { background: #1f2937; border-color: #374151; }
        .dark .rsa-card:hover { border-color: #4b5563; }
        ...
        .rsa-meter i { display: block; height: 100%; border-radius: 999px; background: #22c55e; transition: width .3s ease; }
        ...
        .rsa-section-title { display: flex; align-items: baseline; justify-content: space-between; gap: .75rem; font-size: .9375rem; font-weight: 600; color: #111827; margin: 1.5rem 0 .75rem; }
        .dark .rsa-section-title { color: #f9fafb; }
        .rsa-section-title small { font-size: .75rem; font-weight: 500; color: #6b7280; }
        .dark .rsa-section-title small { color: #9ca3af; }

        /* ---- Team panel ---- */
        .rsa-stats { display: grid; grid-template-columns: repeat(auto-fit, minmax(160px, 1fr)); gap: .75rem; margin-bottom: 1.5rem; }
        .rsa-stat { border: 1px solid #e5e7eb; border-radius: .625rem; padding: .85rem 1rem; }
        .dark .rsa-stat { border-color: #374151; }
        .rsa-stat b { display: block; font-size: 1.375rem; font-weight: 700; color: #111827; line-height: 1.2; }
        .dark .rsa-stat b { color: #f9fafb; }
        .rsa-stat span { font-size: .75rem; color: #6b7280; }
        .dark .rsa-stat span { color: #9ca3af; }

        .rsa-people { list-style: none; margin: 0; padding: 0; }
        .rsa-person { display: flex; align-items: center; gap: .75rem; padding: .7rem 0; border-bottom: 1px solid #f3f4f6; }
        .dark .rsa-person { border-color: #37415188; }
        .rsa-people > li:last-child .rsa-person { border-bottom: none; }
        .rsa-avatar { flex: 0 0 auto; width: 2.25rem; height: 2.25rem; border-radius: 999px; display: grid; place-items: center; font-size: .75rem; font-weight: 700; color: #fff; background: #9ca3af; text-transform: uppercase; }
        .rsa-person-body { min-width: 0; flex: 1; }
        .rsa-person-name { font-size: .8125rem; font-weight: 600; color: #111827; display: flex; align-items: center; gap: .4rem; flex-wrap: wrap; }
        .dark .rsa-person-name { color: #f9fafb; }
        .rsa-person-meta { font-size: .75rem; color: #6b7280; margin-top: .1rem; word-break: break-word; }
        .dark .rsa-person-meta { color: #9ca3af; }
        .rsa-person-side { flex: 0 0 auto; text-align: right; font-size: .7rem; color: #9ca3af; }

        /* ---- Inbox panel ---- */
        .rsa-inbox-frame { height: 70vh; min-height: 480px; }

        /* Embedded inside Settings: that page's own .setting-content
           already supplies padding (1.5rem) and a card background, so
           this wrapper's own padding/max-width would only double up. */
        .rsa-wrap.rsa-embedded { padding: 0; max-width: none; margin: 0; }
    </style>
@endpush
...
@php
    ...
    $seats = $overview['seats'] ?? ['used' => null, 'cap' => null, 'reached' => false, 'known' => false];
    $team = $overview['team'] ?? ['matched' => [], 'resayil_only' => [], 'travelerp_only' => [], 'counts' => ['agents' => 0, 'matched' => 0, 'resayil_only' => 0, 'travelerp_only' => 0]];
    ...
    $initials = function (?string $name) {
        ...
        $letters = strtoupper(($parts[0][0] ?? '').($parts[1][0] ?? ''));

        return $letters !== '' ? $letters : strtoupper(substr($name, 0, 2));
    };

    // A short, closed palette rather than trusting Resayil's free-text
    // `color` field directly as CSS (defence in depth — this value comes
    // from an upstream API response).
    $avatarPalette = ['#6366f1', '#0ea5e9', '#14b8a6', '#f59e0b', '#ef4444', '#8b5cf6', '#ec4899', '#22c55e'];
    $avatarColor = function (string $seed) use ($avatarPalette) {
        return $avatarPalette[crc32($seed) % count($avatarPalette)];
    };
@endphp

<div class="rsa-wrap {{ $embedded ? 'rsa-embedded' : '' }}"
     x-data="resayilAdmin({
        panel: '{{ $activePanel }}',
    ...
    @else
        ...
        <div class="rsa-tabs">
            ...
            <button type="button" class="rsa-tab" x-bind:class="panel === 'team' && 'rsa-tab-active'"
                    x-on:click="show('team')">Team</button>
            <button type="button" class="rsa-tab" x-bind:class="panel === 'inbox' && 'rsa-tab-active'"
                    x-on:click="show('inbox')">Inbox</button>
        </div>

        {{-- ============================ PANEL 1 — OVERVIEW ============================ --}}
        <div x-show="panel === 'overview'" class="rsa-panel">
            <div class="rsa-grid">
            ...
            <div class="rsa-grid" style="margin-top:1rem;">
                <div class="rsa-card">
                    <h3>Team seats</h3>
                    @if($seats['known'] ?? false)
                        @php
                            $used = (int) $seats['used'];
                            $cap = (int) $seats['cap'];
                            $overBy = max(0, $used - $cap);
                            $pct = $cap > 0 ? min(100, (int) round(($used / $cap) * 100)) : 100;
                        @endphp
                        <div class="rsa-card-lead">{{ $used }} of {{ $cap }} included seats</div>
                        <div class="rsa-meter {{ $seats['reached'] ? 'rsa-meter-full' : '' }}"><i style="width: {{ max(2, $pct) }}%"></i></div>
                        @if($overBy > 0)
                            {{-- Honest about the mismatch rather than a
                                 confident "limit reached" alarm: City
                                 Travelers reads 9 people against 8 included
                                 seats, and the extra one is very likely the
                                 workspace owner, who does not appear to
                                 consume a seat — but that is an inference,
                                 not a documented API guarantee, so it is
                                 worded as "likely" here too. --}}
                            <p class="rsa-sub">{{ $used }} people have WhatsApp access; your plan includes {{ $cap }} seats. The difference is most likely the workspace owner, who typically doesn't count against the seat total.</p>
                        @elseif($seats['reached'])
                            <p class="rsa-sub">You've used all included WhatsApp seats. Contact your account manager to add more.</p>
                        @else
                            <p class="rsa-sub">These are the seats included with your plan and how many are in use.</p>
                        @endif
                        <button type="button" class="rsa-btn-link" style="margin-top:.6rem;" x-on:click="show('team')">See who has access &rarr;</button>
                    @else
                        <div class="rsa-card-lead">—</div>
                        <p class="rsa-sub">Seats appear once your WhatsApp number is connected.</p>
                    @endif
                </div>
        ...
        {{-- ============================ PANEL 4 — BILLING ============================ --}}
        <div x-show="panel === 'billing'" x-cloak class="rsa-panel">
            <div class="rsa-card">
                <h3>Subscription</h3>
                @if($subscription)
                    <dl>
        ...
        {{-- ============================ PANEL — TEAM ============================ --}}
        <div x-show="panel === 'team'" x-cloak class="rsa-panel">
            @if(! $device)
                <div class="rsa-empty">Team information appears once your WhatsApp number is connected.</div>
            @else
                <div class="rsa-stats">
                    <div class="rsa-stat"><b>{{ $team['counts']['matched'] }}</b><span>Staff with WhatsApp access</span></div>
                    <div class="rsa-stat"><b>{{ $team['counts']['resayil_only'] }}</b><span>WhatsApp logins we can't match</span></div>
                    <div class="rsa-stat"><b>{{ $team['counts']['travelerp_only'] }}</b><span>Portal staff without access</span></div>
                </div>

                <h2 class="rsa-section-title">Staff with WhatsApp access</h2>
                <div class="rsa-card">
                    @if(empty($team['matched']))
                        <div class="rsa-empty">No portal accounts are matched to a WhatsApp login yet.</div>
                    @else
                        <ul class="rsa-people">
                            @foreach($team['matched'] as $pair)
                                @php $agent = $pair['agent']; $displayName = $pair['user_name'] ?: ($agent['display_name'] ?? $pair['user_email']); @endphp
                                <li>
                                    <div class="rsa-person">
                                        <span class="rsa-avatar" style="background: {{ $avatarColor($pair['user_email']) }};">{{ $agent['initials'] ?: $initials($displayName) }}</span>
                                        <div class="rsa-person-body">
                                            <div class="rsa-person-name">
                                                {{ $displayName }}
                                                <span class="rsa-pill rsa-pill-info">{{ $pair['user_role'] }}</span>
                                                @if($agent['role'])
                                                    <span class="rsa-pill rsa-pill-neutral">WhatsApp {{ ucfirst($agent['role']) }}</span>
                                                @endif
                                            </div>
                                            <div class="rsa-person-meta">{{ $pair['user_email'] }}</div>
                                        </div>
                                        <div class="rsa-person-side">
                                            @if($agent['last_seen_at'] ?? $agent['last_login_at'] ?? null)
                                                Active {{ $fmt($agent['last_seen_at'] ?? $agent['last_login_at'], true) }}
                                            @endif
                                        </div>
                                    </div>
                                </li>
                ...
                @if(!empty($team['resayil_only']))
                    <h2 class="rsa-section-title">
                        WhatsApp logins we can't match
                        <small>Doesn't match a TravelERP account by email</small>
                    </h2>
                    <div class="rsa-card">
                        <ul class="rsa-people">
                            @foreach($team['resayil_only'] as $agent)
                                <li>
                                    <div class="rsa-person">
                                        <span class="rsa-avatar" style="background: {{ $avatarColor($agent['email'] ?: ($agent['display_name'] ?? 'agent')) }};">{{ $agent['initials'] ?: $initials($agent['display_name']) }}</span>
                                        <div class="rsa-person-body">
                                            <div class="rsa-person-name">
                                                {{ $agent['display_name'] ?: ($agent['email'] ?: 'Unnamed') }}
                                                @if($agent['role'])
                                                    <span class="rsa-pill rsa-pill-neutral">WhatsApp {{ ucfirst($agent['role']) }}</span>
                                                @endif
                                            </div>
                                            <div class="rsa-person-meta">{{ $agent['email'] ?: 'No email on file' }}</div>
                                        </div>
                                    </div>
                                </li>
                            @endforeach
                        ...
                        <p class="rsa-sub" style="margin-top:.5rem;">
                            If one of these is a portal user signing in under a different address, we can link the two — just tell us which email pairs with which.
                        </p>
                    </div>
                @endif

                @if(!empty($team['travelerp_only']))
                    <h2 class="rsa-section-title">
                        Portal staff without WhatsApp access
                        <small>{{ count($team['travelerp_only']) }} {{ \Illuminate\Support\Str::plural('person', count($team['travelerp_only'])) }}</small>
                    </h2>
                    <div class="rsa-card">
                        <ul class="rsa-people">
                            @foreach($team['travelerp_only'] as $user)
                                <li>
                                    <div class="rsa-person">
                                        <span class="rsa-avatar" style="background: {{ $avatarColor($user['email']) }};">{{ $initials($user['name']) }}</span>
                                        <div class="rsa-person-body">
                                            <div class="rsa-person-name">
                                                {{ $user['name'] }}
                                                <span class="rsa-pill rsa-pill-neutral">{{ $user['role'] }}</span>
                                            </div>
                                            <div class="rsa-person-meta">{{ $user['email'] }}</div>
                                        </div>
                                    </div>
                                </li>
                            @endforeach
        ...
        {{-- ============================ PANEL — INBOX ============================ --}}
        <div x-show="panel === 'inbox'" x-cloak class="rsa-panel">
            <p class="rsa-sub" style="margin-bottom:.75rem;">
                Your WhatsApp conversations, without leaving Settings. The same inbox also opens from the chat icon anywhere in the app.
            </p>
            <template x-if="inboxMounted">
                <x-resayil-frame :embed-url="$embedUrl" :not-configured="$notConfigured" class="rsa-inbox-frame" />
            </template>
        </div>
...
{{--
    ...
    x-data is evaluated. Guarded against re-declaration: when embedded
    inside Settings, only ONE of the two include sites (standalone page
    vs settings tab) is ever actually rendered per request, but the
    guard costs nothing and protects against a future third include.
--}}
<script>
    if (typeof window.resayilAdmin === 'undefined') {
    window.resayilAdmin = function resayilAdmin(config) {
        return {
            ...
            // Inbox tab (redesign §1): the iframe is only ever inserted into
            // the DOM once the visitor actually opens this tab — never on a
            // plain page load — so browsing Overview/Billing/Team never
            // silently starts loading wa.resayil.io in the background.
            inboxMounted: (config.panel === 'inbox'),
            ...
            show(panel) {
                this.panel = panel;
                if (panel === 'inbox') { this.inboxMounted = true; }
                try {
