<x-app-layout>
    ...
    <div id="setting-index" x-data="settingsPage()" class="bg-white dark:bg-gray-800 rounded-xl shadow-sm flex min-h-[500px]">
        <!-- Internal Sidebar -->
        <div class="w-56 border-r border-gray-200 dark:border-gray-700 p-4 flex-shrink-0">
            <nav class="space-y-1">
                ...
                <!-- Terms & Regulation -->
                <button
                    @click="activeTab = 'terms'; loadTemplates()"
        ...
        <div class="flex-1 p-6">
                                    ...
                                    <td class="px-4 py-4">
                                        <div class="flex items-center justify-end gap-1">
                                            ...
                                            <form x-show="!template.is_default && template.is_active"
                                                :action="`{{ url('terms/templates') }}/${template.id}/set-default`"
                                            ...
                                            <!-- Toggle Active -->
                                            <form :action="`{{ url('terms/templates') }}/${template.id}/toggle-active`"
                                                method="POST" class="inline">
                                                @csrf
                                                <button type="submit"
            ...
            >
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col mx-4"
                    ...
                    <form :action="`{{ url('terms/templates') }}/${editingTemplate?.id}`" method="POST">
                        @csrf
                        @method('PUT')
                        <!-- Modal Header -->
                        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
            ...
            <div x-show="showDeleteModal" x-cloak
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-md mx-4 p-6"
                    ...
                    <form :action="`{{ url('terms/templates') }}/${deletingTemplate?.id}`" method="POST">
                        @csrf
                        @method('DELETE')
            ...
            if (expiryInput) {
                ...
                expiryInput.addEventListener('change', function() {
                    ...
                    fetch('{{ route("settings.invoice.update-expiry") }}', {
                        ...
                        .then(data => {
                            const customSuccessAlert = document.getElementById('custom-success-alert');
