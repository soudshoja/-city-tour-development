<x-app-layout>
    <div class="w-full bg-white rounded-md p-4 mb-4">
        <p>PreBook for {{ $tboPreBook->booking_code }}</p>
    </div>
    ...
    <form method="POST" action="{{route('suppliers.tbo.book') }}" class="bg-white rounded-md p-4">
        ...
        <button type="submit" class="bg-black text-white rounded-md p-2" >Book</button>
    </form>
</x-app-layout>