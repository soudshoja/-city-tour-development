@if($task->originalTask?->status === 'confirmed' && $task->status === 'issued' && $task->originalTask->invoiceDetail)
@php
        ...
        <div @click.stop class="relative w-full max-w-lg rounded-xl bg-white p-6 shadow-2xl dark:bg-gray-800">
            ...
            <div class="mb-5 flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-400">Reference: {{ $task->reference }}</p>
                </div>
            </div>
            ...
            <div class="mb-5 space-y-3">
                <a
                    href="{{ route('invoice.details', [$invoice->agent->branch->company_id, $invoice->invoice_number]) }}"
                    target="_blank"
                    ...
                    <div class="mb-2 flex items-center justify-between">
                        ...
                        <span class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium {{ $isPaid ? 'bg-green-100 text-green-700 dark:bg-green-800/50 dark:text-green-300' : 'bg-yellow-100 text-yellow-700 dark:bg-yellow-800/50 dark:text-yellow-300' }}">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div class="rounded-lg border border-red-200 bg-red-50 p-3 dark:border-red-800/50 dark:bg-red-900/20">
                        <div class="mb-2 flex items-center justify-between">
                            ...
                            <span class="inline-flex items-center rounded-full bg-red-100 px-2 py-0.5 text-xs font-medium text-red-700 dark:bg-red-800/50 dark:text-red-300">
                                {{ ucfirst($task->originalTask->status) }}
                            </span>
                        </div>
                        <p class="text-sm font-medium text-gray-900 dark:text-white">Task #{{ $task->originalTask->id }}</p>
                        <p class="text-xs text-gray-500 dark:text-gray-400">{{ $task->originalTask->reference }}</p>
                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Cost: <span class="font-medium">{{ number_format($oldSupplierCost, 3) }} {{ $invoice->currency ?? 'KWD' }}</span></p>
                    </div>
                    ...
                    <div class="rounded-lg border border-green-200 bg-green-50 p-3 dark:border-green-800/50 dark:bg-green-900/20">
                        ...
                        <p class="text-sm font-medium text-gray-900 dark:text-white">Task #{{ $task->id }}</p>
                        <p class="text-xs text-gray-500 dark:text-gray-400">{{ $task->reference }}</p>
                        <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Cost: <span class="font-medium">{{ number_format($newSupplierCost, 3) }} {{ $invoice->currency ?? 'KWD' }}</span></p>
                    </div>
                </div>

                @if($isPaid && $hasPriceChange)
                <div class="rounded-lg border {{ $isLoss ? 'border-red-300 bg-red-50 dark:border-red-700 dark:bg-red-900/30' : 'border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-900/50' }} p-3">
                    <p class="text-xs font-medium uppercase tracking-wide {{ $isLoss ? 'text-red-600 dark:text-red-400' : 'text-gray-500 dark:text-gray-400' }} mb-2">Profit Impact</p>
                    <div class="grid grid-cols-3 gap-2 text-center">
                        <div>
                            <p class="text-xs text-gray-500 dark:text-gray-400">Selling Price</p>
                            <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ number_format($taskPrice, 3) }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-500 dark:text-gray-400">Old Profit</p>
                            <p class="text-sm font-semibold {{ $oldProfit >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">{{ number_format($oldProfit, 3) }}</p>
                        </div>
                        <div>
                            <p class="text-xs text-gray-500 dark:text-gray-400">New Profit</p>
                            <p class="text-sm font-semibold {{ $newProfit >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">{{ number_format($newProfit, 3) }}</p>
                        </div>
                    </div>
                    @if($isLoss)
                    <div class="mt-3 flex items-center gap-2 rounded-lg bg-red-100 p-2 dark:bg-red-800/50">
                        ...
                        <p class="text-xs font-medium text-red-700 dark:text-red-300">Warning: This will result in a LOSS of {{ number_format(abs($newProfit), 3) }} {{ $invoice->currency ?? 'KWD' }}</p>
                    </div>
                    ...
                    <div class="mt-3 flex items-center gap-2 rounded-lg bg-amber-100 p-2 dark:bg-amber-800/50">
                        ...
                        <p class="text-xs font-medium text-amber-700 dark:text-amber-300">Profit will decrease by {{ number_format(abs($profitDifference), 3) }} {{ $invoice->currency ?? 'KWD' }}</p>
                    </div>
                    @endif
                </div>
                @endif
            ...
            <div class="flex items-center justify-end gap-3">
                ...
                <form method="POST" action="{{ route('tasks.switchInvoice', $task) }}" class="inline-block">
                    @csrf
                    <button
                        type="submit"
                        class="inline-flex items-center gap-2 rounded-lg {{ $isPaid && $hasPriceChange && $isLoss ? 'bg-red-600 hover:bg-red-700 focus:ring-red-500' : 'bg-blue-600 hover:bg-blue-700 focus:ring-blue-500' }} px-4 py-2 text-sm font-medium text-white transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 dark:focus:ring-offset-gray-800">
                        <svg class="size-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4" />
                        </svg>
                        {{ $isPaid && $hasPriceChange && $isLoss ? 'Switch Anyway' : 'Switch Invoice' }}
                    </button>
                </form>
            </div>
        </div>
