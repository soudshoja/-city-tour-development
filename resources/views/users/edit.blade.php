<x-app-layout>
    <nav class="flex items-center space-x-2 rtl:space-x-reverse text-sm mb-4 sm:mb-6 overflow-x-auto">
        <a href="{{ route('users.index') }}" class="text-gray-500 hover:text-gray-700 transition whitespace-nowrap">Users</a>
        <span class="text-gray-400">&gt;</span>
        <span class="text-blue-600 font-medium truncate max-w-[200px] sm:max-w-none">{{ $user->name }}</span>
    </nav>
                        ...
                        @if ($role->id != 1)
                            <label class="relative cursor-pointer">
                                <input type="radio" name="role_id" value="{{ $role->id }}" 
                                    data-role-name="{{ strtolower($role->name) }}" class="role-radio peer sr-only"
            ...
            <form action="{{ route('users.updateInfo', $user) }}" method="POST" id="info-form">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        @if(isset($userRole) && in_array($userRole, ['accountant']))
                            <div class="flex gap-2">
                                <div class="w-28">
                                    <input type="text" name="country_code" id="info-country-code" 
                                        ...
                                        value="{{ $countryCode ?? '' }}" placeholder="+965">
                                </div>
                                <div class="flex-1">
                            ...
                            <p class="text-xs text-gray-500 mt-1">
                                ...
                                Country code and phone number are separate for {{ $userRole }}s
                            </p>
                        @else
                            <input type="text" name="phone" id="info-phone" class="form-input w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500" 
                                value="{{ $phone ?? '' }}" placeholder="Phone number with country code">
                        @endif
                    </div>
