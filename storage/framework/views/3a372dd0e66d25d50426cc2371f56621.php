...
<div class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900 bg-opacity-30" 
    ...
    <div class="bg-white rounded-lg shadow-xl p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto"
        ...
        <div class="mb-6">
            <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                ...
                <div class="text-sm text-blue-800 space-y-1">
                    <p><strong><?php echo e(session('duplicate_data.duplicate_message')); ?></strong></p>
                    <?php if(session('duplicate_data.existing_client')): ?>
                    <p><strong>Name:</strong> <?php echo e(session('duplicate_data.existing_client.full_name')); ?></p>
                    <p><strong>Phone:</strong> <?php echo e(session('duplicate_data.existing_client.phone')); ?></p>
                    <p><strong>Email:</strong> <?php echo e(session('duplicate_data.existing_client.email') ?? 'Not provided'); ?></p>
                    <?php if(session('duplicate_data.existing_client.civil_no')): ?>
                    <p><strong>Civil No:</strong> <?php echo e(session('duplicate_data.existing_client.civil_no')); ?></p>
                    <?php endif; ?>
                    <?php endif; ?>
                </div>
            </div>
                    ...
                    <div class="flex-shrink-0">
                        <div class="w-12 h-12 bg-amber-500 rounded-full flex items-center justify-center">
                            <span class="text-white font-semibold text-lg">
                                <?php echo e(substr(session('duplicate_data.owner_agent.name'), 0, 1)); ?>

                            </span>
                        </div>
                    </div>
                    <div class="ml-4">
                        <p class="text-sm font-semibold text-amber-900"><?php echo e(session('duplicate_data.owner_agent.name')); ?></p>
                        <p class="text-xs text-amber-700">Owner Agent</p>
                        <?php if(session('duplicate_data.owner_agent.branch')): ?>
                        <p class="text-xs text-amber-600 mt-1">Branch: <?php echo e(session('duplicate_data.owner_agent.branch.name') ?? 'N/A'); ?></p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
        ...
        <form action="<?php echo e(route('clients.request-assignment')); ?>" method="POST" id="assignment-request-form">
            <?php echo csrf_field(); ?>
            
            <input type="hidden" name="existing_client_id" value="<?php echo e(session('duplicate_data.existing_client.id')); ?>">
            <input type="hidden" name="owner_agent_id" value="<?php echo e(session('duplicate_data.owner_agent.id')); ?>">
...
<?php endif; ?>
<?php /**PATH /home/soudshoja/soud-laravel/resources/views/components/duplicate-client-warning.blade.php ENDPATH**/ ?>