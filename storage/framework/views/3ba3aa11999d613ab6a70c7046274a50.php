<div id="payment_gateway_section" class="mt-6 bg-white p-6 rounded-lg shadow-md">
    ...
    <div class="mb-4">
        ...
        <select id="payment_gateway_option" name="payment_gateway_option"
            ...
            <option value="<?php echo e($gateway->name); ?>"
                ...
                <?php echo e($selectedPaymentGateway == strtolower(old('payment_gateway_option', $gateway->name)) ? 'selected' : ''); ?>>
                <?php echo e($gateway->name); ?>

                <?php if(isset($gateway->gateway_fee) && $gateway->gateway_fee > 0): ?>
                (Fee: <?php echo e(number_format($gateway->gateway_fee, 2)); ?>)
                <?php endif; ?>
            </option>
    ...
    <input type="hidden" name="service_charge" id="service_charge" value="<?php echo e(old('service_charge', isset($refund) ? ($refund->service_charge ?? 0) : 0)); ?>">
</div>

<script>
    // Payment gateway data from backend
    const paymentGateways = <?php echo json_encode($paymentGateways ?? [], 15, 512) ?>;
    const paymentMethods = <?php echo json_encode($paymentMethods ?? [], 15, 512) ?>;
    let old_payment_gateway = "<?php echo e(old('payment_gateway_option', isset($refund) && $refund->invoice && $refund->invoice->invoicePartials->first() ? strtolower($refund->invoice->invoicePartials->first()->payment_gateway) : '')); ?>";
    let old_payment_method = "<?php echo e(old('payment_method', isset($refund) && $refund->invoice && $refund->invoice->invoicePartials->first() ? (int) $refund->invoice->invoicePartials->first()->payment_method : 0)); ?>";

    // Selected values for editing mode
    const selectedPaymentMethod = <?php echo e(isset($refund) && $refund->invoice && $refund->invoice->invoicePartials->first() ? (int) $refund->invoice->invoicePartials->first()->payment_method : 0); ?>;
...
</script><?php /**PATH /home/soudshoja/soud-laravel/resources/views/refunds/partial/payment-gateway-selection.blade.php ENDPATH**/ ?>