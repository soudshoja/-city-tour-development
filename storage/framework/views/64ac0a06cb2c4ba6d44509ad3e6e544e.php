...
<script>
    ...
    // let agents = [];
    // let clients = [];
    let clientsChat = <?php echo json_encode($clients, 15, 512) ?>;
    let agentsChat = <?php echo json_encode($agents, 15, 512) ?>;
    ...
    sendMessageButton.on("click", function() {
        ...
        fetch("<?php echo e(route('chat.process')); ?>", {
                method: "POST",
                headers: {
    ...
    confirmTasksButton.on("click", function() {
        ...
        fetch("<?php echo e(route('chat.select')); ?>", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "X-CSRF-TOKEN": document.querySelector('meta[name="csrf-token"]').getAttribute(
    ...
    $("#pricing-form").on("submit", function(event) {
        ...
        fetch("<?php echo e(route('chat.create')); ?>", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "X-CSRF-TOKEN": document.querySelector('meta[name="csrf-token"]').getAttribute(
    ...
    $('#passport-upload-input-chat').on('change', function(event) {
        ...
        if (file) {
            ...
            fetch("<?php echo e(route('chat.handleFileUpload')); ?>", {
                    method: "POST",
                    body: formData,
                    headers: {
                        "X-CSRF-TOKEN": document.querySelector('meta[name="csrf-token"]').getAttribute(
    ...
    document.getElementById("client-form").addEventListener("submit", async function(event) {
        ...
        try {
            const response = await fetch("<?php echo e(route('chat.client')); ?>", {
                method: "POST",
                headers: {
    ...
    document.getElementById("agent-form").addEventListener("submit", async function(event) {
        ...
        try {
            const response = await fetch("<?php echo e(route('chat.agent')); ?>", {
                method: "POST",
                headers: {
                    "X-CSRF-TOKEN": csrfToken,
                },
    ...
    document.getElementById("branch-form").addEventListener("submit", async function(event) {
        ...
        try {
            const response = await fetch("<?php echo e(route('chat.branch')); ?>", {
                method: "POST",
                headers: {
                    "X-CSRF-TOKEN": csrfToken,
                },
...
</script><?php /**PATH /home/soudshoja/soud-laravel/resources/views/livewire/chat.blade.php ENDPATH**/ ?>