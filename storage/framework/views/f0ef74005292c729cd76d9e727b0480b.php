...
<div class="space-y-4">
    <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('settingCompanyInvoice', 'App\Models\Setting')): ?>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (expiryInput) {
            expiryInput.value = '<?php echo e($invoiceExpiryDefault); ?>';

            expiryInput.addEventListener('change', function() {
                const newValue = expiryInput.value;
                fetch('<?php echo e(route("settings.invoice.update-expiry")); ?>', {
                        method: 'POST',
                        headers: {
        ...
        if (whatsappCheckbox) {
            whatsappCheckbox.checked = <?php echo e($invoiceWhatsappSetting ? 'true' : 'false'); ?>;

            whatsappCheckbox.addEventListener('change', function() {
                const isEnabled = whatsappCheckbox.checked;
                fetch('<?php echo e(route("user-settings.update")); ?>', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>',
...
</script><?php /**PATH /home/soudshoja/soud-laravel/resources/views/settings/partial/invoice.blade.php ENDPATH**/ ?>