...
class WebhookSigningServiceTest extends TestCase
{
    ...
    public function test_store_secret_returns_encrypted_string()
    {
        $plaintext = 'my-plaintext-secret-12345';

        $stored = $this->service->storeSecret($plaintext);

        $this->assertIsString($stored);
        // Encrypted value must not equal the plaintext
        $this->assertNotEquals($plaintext, $stored);
        // Laravel encrypt produces a non-empty string longer than plaintext
        $this->assertGreaterThan(strlen($plaintext), strlen($stored));
    }

    public function test_retrieve_secret_decrypts_stored_value()
    {
        $plaintext = 'my-plaintext-secret-12345';

        $stored = $this->service->storeSecret($plaintext);
        $retrieved = $this->service->retrieveSecret($stored);

        $this->assertEquals($plaintext, $retrieved);
    }

    public function test_store_retrieve_roundtrip_is_lossless()
    {
        $secrets = [
            'simple',
        ...
        foreach ($secrets as $s) {
            $this->assertEquals($s, $this->service->retrieveSecret($this->service->storeSecret($s)));
        }
    }

    public function test_two_encryptions_of_same_plaintext_produce_different_ciphertexts()
    {
        // Crypt::encrypt uses a random IV, so same plaintext → different ciphertext each time
        $plaintext = 'same-secret';

        $stored1 = $this->service->storeSecret($plaintext);
        $stored2 = $this->service->storeSecret($plaintext);

        $this->assertNotEquals($stored1, $stored2);
        // But both decrypt to the same plaintext
        $this->assertEquals($plaintext, $this->service->retrieveSecret($stored1));
        $this->assertEquals($plaintext, $this->service->retrieveSecret($stored2));
    }
}
